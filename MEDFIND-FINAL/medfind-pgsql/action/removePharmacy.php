...
<?php
$pharm_id = $_GET['id'];
		$sql1 = "select * from users where pharmacy = $pharm_id";
		...
		if(pg_num_rows($notdelete) >= 1) { ?> 
			...
			<div class="popup__content">
			  <h3 class="popup__content__title">
				Pharmacy Active. Do not remove.
			  </h1>
			  <p>Error Removing Pharmacy</p>
			  <p>
			  <a href="../super-pharmacy.php"><button class="button button--error" data-for="js_error-popup">Close</button></a>
			  </p>
			</div>
		  </div>
		<?php } else{

 $sql = "CALL remove_pharmacy($pharm_id)";
 $delete = pg_query($dbconn, $sql);
	//echo "<script> alert('Medicine Removed Successfully'); window.location='../super-product.php'; </script>";
                echo '<div class="popup popup--icon -success js_success-popup popup--visible">
                        ...
                        <div class="popup__content">
                        <h3 class="popup__content__title">
                        Pharmacy Successfully Removed 
                </h1>
                <p>Pharmacy Successfully Removed</p>
                <p> ';
                echo  "<script>setTimeout(\"location.href = '../super-pharmacy.php';\",1500);</script>";
