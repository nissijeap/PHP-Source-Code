...
if($_POST) {    
  ...
  if(empty($email) || empty($password)) {
    ...
    $sql = "SELECT * from get_users('$email', '$password')";
    $result = pg_query($dbconn,$sql);
    $row = pg_num_rows($result);

    if($row == 1) {
      // exists
      $mainSql = "SELECT * from get_users('$email', '$password')";
      $mainResult = pg_query($dbconn,$mainSql);
      $row1 = pg_num_rows($mainResult);

      if($row1 == 1) {
        $value = pg_fetch_assoc($mainResult);
        $user_id = $value['usrs_id'];
  ...
  <div class="popup__content">
    ...
    <p>
     ...
     if($email == 'superadmin@gmail.com'){
      echo "<script>setTimeout(\"location.href = 'super-dashboard.php';\",1500);</script>";
     } else {
