    ...
    for($i=0;$i<$max;$i++){
      if($pid==$_SESSION['admin_gcCart'][$i]['productID']){
          if($q>0  && $q<=999){
            ...
            if ( $rescart->qty<=$q) { 
             $_SESSION['admin_gcCart'][$i]['qty']=  $rescart->qty;
...
// saving mutiple products
    ...
    function admin_multi_product_exists($pid,$q){
    // $pid=intval($pid); 
    $max=count($_SESSION['admin_multi_gcCart']);
    $flag=0;
    for($i=0;$i<$max;$i++){
      if($pid==$_SESSION['admin_multi_gcCart'][$i]['productID']){
          if($q>0  && $q<=999){
            # code...
            $flag=1;
             $_SESSION['admin_multi_gcCart'][$i]['qty']= $_SESSION['admin_multi_gcCart'][$i]['qty'] + $q;
             $_SESSION['admin_multi_gcCart'][$i]['subtotal']= $_SESSION['admin_multi_gcCart'][$i]['price'] * $_SESSION['admin_multi_gcCart'][$i]['qty'];
              // message("{$q} Item added in the cart.","success");
              break;
          }
        $flag=1;
 ...
 function admin_multi_addtocart($pid,$product,$price,$q,$subtotal,$date_expired){
    if($pid<1 or $q<1) return;
    if($q<1) return;
    if (!empty($_SESSION['admin_multi_gcCart'])){


    if(is_array($_SESSION['admin_multi_gcCart'])){
      if(admin_multi_product_exists($pid,$q)) return;
      $max=count($_SESSION['admin_multi_gcCart']);
      $_SESSION['admin_multi_gcCart'][$max]['productID']=$pid;
      $_SESSION['admin_multi_gcCart'][$max]['product']=$product;
      $_SESSION['admin_multi_gcCart'][$max]['qty']=$q;
      $_SESSION['admin_multi_gcCart'][$max]['price']=$price;
      $_SESSION['admin_multi_gcCart'][$max]['subtotal']=$subtotal;
      $_SESSION['admin_multi_gcCart'][$max]['date_expired']=$date_expired;
    }
    else{
     $_SESSION['admin_multi_gcCart']=array();
      $_SESSION['admin_multi_gcCart'][0]['productID']=$pid;
      $_SESSION['admin_multi_gcCart'][0]['product']=$product;
      $_SESSION['admin_multi_gcCart'][0]['qty']=$q;
      $_SESSION['admin_multi_gcCart'][0]['price']=$price;
      $_SESSION['admin_multi_gcCart'][0]['subtotal']=$subtotal;
      $_SESSION['admin_multi_gcCart'][0]['date_expired']=$date_expired;
    }
}else{
     $_SESSION['admin_multi_gcCart']=array();
      $_SESSION['admin_multi_gcCart'][0]['productID']=$pid;
...
function admin_multi_removetocart($pid){
  // $pid=intval($pid);
  $max=count($_SESSION['admin_multi_gcCart']);
  for($i=0;$i<$max;$i++){
    if($pid==$_SESSION['admin_multi_gcCart'][$i]['productID']){
      unset($_SESSION['admin_multi_gcCart'][$i]);
      break;
    }
  }
  $_SESSION['admin_multi_gcCart']=array_values($_SESSION['admin_multi_gcCart']);
}


 function admin_multi_editproduct($pid,$q){
    // $pid=intval($pid); 
  if($q<1) return;
    if (!empty($_SESSION['admin_multi_gcCart'])){
       if(is_array($_SESSION['admin_multi_gcCart'])){
          ...
          for($i=0;$i<$max;$i++){
            if($pid==$_SESSION['admin_multi_gcCart'][$i]['productID']){
                if($q>0  && $q<=999){
                  # code...
                  $flag=1;
                   $_SESSION['admin_multi_gcCart'][$i]['qty']= $q;
                   $_SESSION['admin_multi_gcCart'][$i]['subtotal']= $_SESSION['admin_multi_gcCart'][$i]['price'] * $_SESSION['admin_multi_gcCart'][$i]['qty'];
                    break;
                }
              // $flag=1;
