...
$res = $pdo->exec("UPDATE `orders` SET `customer`='".$customer."', `tracking_number`='".$trackingnum."', `description`='".$desc."',`received_by`='".$received_by."',`amount`='".$amount."',`paid`='".$paid."',`date_received`='".$date_received."',`completed`='".$completed."',`date_collected`='".$date_collected."' WHERE id='".$eid."'");
$res2 = $pdo->exec("UPDATE `calendar` SET `title`='".$name."', `description`='".$desc."', `start`='".$date_received."', `end`='".$date_collected."', `allDay`='true', `color`='".$color."', `url`='../orderedit.php?id=$eid', `category`='Orders', `user_id`='".$uid."' WHERE `orders`='".$eid."'");
...
<?php
$oldd = $pdo->query("SELECT * FROM `orders` WHERE id='".$eid."'");
...
?>
				...
				<div class="form-group">
					<label>Tracking Number</label><br/>
                 	<input type="text" name="tracking_number" style="width:200px; height: 40px;" value="<?php echo($old['tracking_number']) ?>" /><br/><br/>
				</div>
