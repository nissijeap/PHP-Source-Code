...
INSERT INTO `general_setting` (`id`, `sitename`, `address`, `email`, `mobile`, `currency`, `sms`) VALUES
(1, 'Pink Blush Fashion and Merchandise', '66GQ+VRM, Badelles St, Iligan City, Lanao del Norte', 'pinkblush@gmail.com', '09068169676', 'PHP', 'http://sms.marketnaija.com//API/?action=compose&username=mamama&api_key=aaaaaa999999ddddd&sender=sarutech&to=[TO]&message=[MESSAGE] Just Fill Up the full text from your sms api service with replace message with[MESSAGE] and To-Phonenumber with [TO]');
...
ALTER TABLE `orders` ADD UNIQUE(`tracking_number`);
-- --------------------------------------------------------

--
-- Table structure for trigger `balance`
--
...
ON orders
FOR EACH ROW
SET new.balance = new.amount - new.paid;
...
(3, 'staff', 'de9bf5643eabf80f4a56fda3bbb84483');
